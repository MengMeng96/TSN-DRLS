...
class Environment:
    ...
    def translate_data_to_inputs(self):
        ...
        # 在PCL的TSN环境中，由于无法使用离散的时隙，因此无法使用LD时隙选择方法，也就不需要time_slot_status信息了
        for edge in self.graph.edges.values():
        ...
        for edge in self.graph.edges.values():
            ...
            # 由于是先找路由，后统一找时隙，因此这个时隙和分数信息也不需要了
            time_slot, score = 0, 0
                # edge.find_time_slot(start_time, offset, self.tt_flow_cycle, self.tt_flow_length, self.tt_flow_deadline)
            if time_slot < 0:
                is_source = 0 # 将超时因素也考虑进去
            time_slot_num = 0 # self.avaiable_time_slot_number(edge)
            ...
            policy_inputs[edge.id, 4] = sum(edge.start_port.port_status) / len(edge.start_port.port_status) - total_usage
            policy_inputs[edge.id, 5] = (score + 120) / 120
            policy_inputs[edge.id, 6] = (self.tt_flow_deadline - self.time) / self.tt_flow_deadline
            # 当前边的出度，出度越小，更有可能成为一个拥挤边
    ...
    # time_slot_status是用来表示由多少个可用的时隙
    # 这里的可用不仅仅是指当前时隙是空闲的，更包含了在一个弘周期内当前TT流需要的所有时隙都是空闲的
    # 在PCL-TSN不等待情况下，这一信息用不上
    # TODO 这个信息也需要重新写，time_slot_status信息要写在port中
