        ...
        for i in range(self.node_num):
            ...
            for port_id in range(0, args.per_node_port_count):
                ports.append({"id": port_id,
                              ...
                              "queues": []})
                for queue_id in range(0, args.all_queue_count):
                    ports[port_id]["queues"].append({"id": queue_id,
                                                     "occupiedTimeIntervals": []})
            cur_node["ports"] = ports
            self.node_info.append(cur_node)

    def transform_schedule_to_node_info(self, schedule_result):
        for node_info in schedule_result:
            node_id = node_info["nodeId"]
            for port_info in node_info["ports"]:
                port_id = port_info["portId"]
                start_time = 0
                for gcl_info in port_info["gcls"]:
                    end_time = start_time + gcl_info["timeDuration"]
                    queue_status = gcl_info["gateStates"]
                    for queueID in range(0, args.all_queue_count):
                        if queue_status[queueID] == 1:
                            self.node_info[node_id]["ports"][port_id]["queues"][queueID]["occupiedTimeIntervals"].append({"startTime": start_time,
                                                              "endTime": end_time})
                    start_time = end_time
        self.input = {"id": 0,
    ...
    def gene_all(self, node_num = 10, port_num = 5, eps = 0.2,
        ...
        self.input = {"id": 0,
                      "topology": {"links": self.link_info,
                                   "nodes": self.node_info},
                      "flows": self.tt_flow}
        print("function RingSpecific gene_all finish")
